...
async def process_counter_request(m: Message, hero_name: str):
    ...
    loading_msg = await m.answer(f"🛡 <b>Analis AI mencari counter {real_name}...</b>")
    
    payload = {"type": "counter", "hero": hero_data}
...
async def counter_cmd(m: Message):
    ...
    if len(args) < 2:
        await m.answer("⚠️ Gunakan: <code>/counter NamaHero</code>")
        return
